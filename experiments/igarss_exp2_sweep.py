...
if __name__ == "__main__":
    ...
    timing_stats = {m: {"base": [], "dtl": [], "matl": [], "tg_matl": []} for m in ["clip", "dinov2", "mae"]}
    results_stats = {m: {"base": [], "dtl": [], "matl": [], "tg_matl": []} for m in ["clip", "dinov2", "mae"]}
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_rgb_trainval, composite_label_encoded_trainval)):
        ...
        tp_values = [40, 50, 60, 70]
        rp_values = [5, 10, 15, 20, 25, 30]
        ...
        for experiment_used in ["clip", "dinov2", "mae"]:
            ...
            if experiment_used == 'clip':
                base_train_embeddings, base_val_embeddings, base_test_embeddings = (
                    awir_clip_emb_train, awir_clip_emb_val, awir_clip_emb_test
                )
            elif experiment_used == 'dinov2':
                base_train_embeddings, base_val_embeddings, base_test_embeddings = (
                    awir_dinov2_emb_train, awir_dinov2_emb_val, awir_dinov2_emb_test
                )
            elif experiment_used == 'mae':
                base_train_embeddings, base_val_embeddings, base_test_embeddings = (
                    awir_mae_emb_train, awir_mae_emb_val, awir_mae_emb_test
                )
            ...
            for tp in tp_values:
                for rp in rp_values:
                    ...
                    print(f"Running TP={tp}, RP={rp}", flush=True)

                    # ===========================
                    # Load TG-MATL projection model for this tp/rp combination
                    # ===========================
                    tg_matl_model_path = (
                        f"./results/trained_models/exp1_emb_proj/{experiment_used}/tp{tp}_rp{rp}/"
                        f"tg_matl_best_fold{run_number}.h5"
                    )

                    if not os.path.exists(tg_matl_model_path):
                        print(f"Missing model: {tg_matl_model_path}")
                        continue

                    tg_matl_model = projection_head(base_train_embeddings.shape[1])
                    tg_matl_model.load_weights(tg_matl_model_path)
                    tg_matl_proj_model = tf.keras.Model(
                    ...
                    emb_dict = {"tg_matl": tg_matl_train_embeddings}
                    ...
                    for emb_type, emb_data_train in emb_dict.items():
                        ...
                        class_model.compile(optimizer=Adam(1e-4),
                        ...
                        class_model.fit(
                            ...
                            verbose=0,
                            callbacks=[ModelCheckpoint(class_ckpt, save_best_only=True,
                        ...
                        reg_boxfeat_model.fit(
                            ...
                            batch_size=parameters['batch_size'],
                            verbose=0,
                            callbacks=[ModelCheckpoint(reg_boxfeat_ckpt, save_best_only=True,
                                                      monitor='val_loss', verbose=0)]
                        )
                        ...
                        reg_boxloc_model.fit(
                            ...
                            batch_size=parameters['batch_size'],
                            verbose=0,
                            callbacks=[ModelCheckpoint(reg_boxloc_ckpt, save_best_only=True,
                                                      monitor='val_loss', verbose=0)]
                        )
                        ...
                        # ===========================
                        # Save results with TP/RP
                        # ===========================
                        np.save(
                            os.path.join(
                                results_save_dir,
                                f"{emb_type}_tp{tp}_rp{rp}_fold{run_number}_taskhead_metrics.npy"
                            ),
                            {
                                "acc": acc, "f1": f1,
                                "mse_feat": mse_feat, "r2_feat": r2_feat,
                                "mse_loc": mse_loc, "r2_loc": r2_loc,
